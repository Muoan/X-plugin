...
****
## 😍插件介绍：
**X/Twitter 推文视频/图片/GIF 解析下载插件：发链接自动解析、下载到服务器生成直链分享、内置订阅代理（v2ray）**

**本插件拒绝任何【非合理】提议~如遇到问题请卸载本插件或截图＋文字叙述提交issues或加入QQ群或Yunzai论坛反馈即可~**

**⚠️ 免责声明：** 本插件不对 x.com 内容进行审核处理，所以选择外部链接展示相关内容，请你配置好您的公网 IP 及其端口。本插件对 x.com 进行资源获取分享，不做任何处理，使用本插件时如果导致您的账号封禁·禁言等概不负责。如无法接受请卸载本插件。
...
- [ ] **开放所有用户独立绑定 Cookie 进行 x.com 操作**：目前 Cookie 仅主人可用（评论/用户/时间线/搜索/通知都走主人 Cookie）；后续按 QQ 用户（user_id）各自绑定自己的 X Cookie，`#X搜索`/`#X用户`/`#X查看` 等操作使用各自绑定账号，互不影响、互不可见
- [ ] Windows 平台适配：当前代理（v2ray 路径/ps 命令）与 Chromium 路径为 Linux 部署，Windows 需改路径
- [ ] 搜索会话持久化到面板可见（当前仅 QQ 命令按人隔离，web 面板搜索结果不区分人）
- [ ] 渲染页敏感内容脱敏开关（当前原样展示）
