...
export function renderTweetHtml (tweet, comments) {
  ...
  const comBlock = Array.isArray(comments)
    ? `<div class="thread">${comments.length ? `<div class="th-head"><b>${comments.length}</b> 条回复</div>` : ''}${comments.map(c => {
        const cav = c.avatar_url ? `<img class="c-av" src="/img?u=${encodeURIComponent(c.avatar_url)}" alt="" loading="lazy">` : '<div class="c-av">🧑</div>'
        ...
        return `<div class="c-row"><div class="c-avcol">${cav}<span class="c-line"></span></div><div class="c-b"><p class="c-h"><span class="nm">${esc(c.name || c.screen_name || '')}</span><span class="hd">@${esc(c.screen_name || '')}</span>${ct ? `<span class="tm">· ${ct}</span>` : ''}</p><p class="c-t">${esc(c.text || '')}</p><div class="c-a"><span title="回复">${svg.reply}</span><span title="转发">${svg.repost}</span><span title="喜欢">${svg.like}<b>${fmt(c.likes)}</b></span></div></div></div>`
      }).join('')}</div>`
    : ''
